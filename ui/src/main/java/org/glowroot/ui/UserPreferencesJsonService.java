...
public class UserPreferencesJsonService {
    ...
    @POST(path = "/backend/admin/user-preferences/addFavorite", permission = "admin:edit:userPreferences")
    String addFavorite(@BindRequest AddFavoriteRequest request) throws Exception {
        if (this.accessTokenExpires < System.currentTimeMillis()){
            this.accessToken = getAccessToken();
        }
        String url = "http://localhost:3000/api/external/favorites";
