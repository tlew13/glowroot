...
public class AccessRoleImpl implements IAccessRole {
        ...
        switch(roleType) {
            ...
            case "ADMIN":
                ...
                switch (roleName.toUpperCase()) {
                    case "ALLAPP-USER":
                        permissions.addAll(getDefaultPermissions("*", defaultUserPermissions));
                        permissions.add("agent:*:jvm:threadDump");
                        break;
                    case "ALLAPP-ADMIN":
                        permissions.addAll(Arrays.asList(defaultAdministratorPermissions));
        ...
        try {
            ...
            for (String app : appsAllowed) {
                if (roleType.equalsIgnoreCase("USER")){
                    permissions.addAll(getDefaultPermissions(app, defaultUserPermissions));
                } else { //else ADMIN
                    permissions.addAll(getDefaultPermissions(app, defaultAdminPermissions));
                }
            }
    ...
    private Set<String> getDefaultPermissions(String appName, String[] permissionList){
        Set<String> permissions = new HashSet<>();
        for (String defaultPermission : permissionList) {
            if (appName.equals("*")) {
